	...
	bool bShouldOpenInDefaultsMode)
{
	...
	DialogueDocumentManager = MakeShareable(new FDocumentTracker);
	DialogueDocumentManager->Initialize(SharedThis(this));
	DialogueDocumentManager->SetTabManager(TabManager.ToSharedRef());
...
void FDialogueBuilderCustomEditor::InvokeDialogueBuilderGraphTab()
{
	...
	DialogueDocumentManager->OpenDocument(Payload, FDocumentTracker::OpenNewDocument);
...
{
	return DialogueDocumentManager;
}
